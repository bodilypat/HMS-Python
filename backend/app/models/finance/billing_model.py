...
class BillingModel:
	...
	def create_billing( 
		...
		conn = get_connection()
		if not conn:
            print("[BillingModel] Database connection failed.")
			return None
		...
		try:
			...
			print(f "[BillingModel] Error Creating billing: {e}")
			return None
		finally:
			cursor.close()
			conn.close()
	...
	def get_billing_by_reservation(reservation_id: int) -> Optional[Dict[str, Any]]:
		...
		try:
			...
			print(f"[BillingModel] Error Fetching billing: {e}")
			return None
		finally:
			cursor.close()
			conn.close()
	...
	def update_billing(
		...
		try:
			...
			if not fields:
                print("[BillingModel] No fields provided for update. ")
				return False 
	...
	def delete_billing(billing_id: int) -> bool:
		...
		try:
			...
			print(f"[BillingModel] Error deleting billing: {e}")
			return False
		finally:
			cursor.close()
			conn.close()
