# backend/app/models/RoomServiceModel.py
...
class RoomServiceModel:
	...
	@staticmethod
	def create_room_service(reservation_id: int, service_id: int, service_status="Requested", note=None):
		conn = get_connection()
		if not conn:
			return None
		...
		@staticmethod
		def update_room_service(room_service_id: int, service_status: Optional[str] = None, note: Optional[str] = None) -> bool:
	...
	def list_service_by_reservation(reservation_id: int) -> List[Dict[str, Any]]:
		...
		try:
			cursor = conn.cursor(dictionary=True)
			cursor.execute("SELECT * FROM room_services WHERE reservation_id = %d = %s", (reservation_id,))
