...
router = APIRouter(prefix="/auth", tags=["Autnentication"])
...
def register(user_in: user_schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
        ...
        if not user:
            ...
            access_token = security.create_access_token(data{"sub": str(user.id)}, expire_delta=access_token_expires)
    ...
    def refresh_token(refresh_token: str):
        ...
        access_token_expire = timedelta(minutes=security.ACCESS_TOKEN_EXPIRE_MINUTES)
        new_access_token = security.create_access_token(data={"sub": user_id}, expires_delta=access_token_expires)
