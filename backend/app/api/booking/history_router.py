...
from schemas.booking.history_router import HistoryCreate, HistoryUpdate, HistoryRead 
...
@router.get(
        ...
        description="Retrieve a paginated list of boking history records"
    )
def read_histories(
...
@router.get(
        "/{history_id}"
        response_model=HistoryRead,
        summary="Get a booking history by ID",
        description="Retrieve a specific booking history record using its ID."
    )
def read_history(
        history_id: int,
        db: Session = Depends(get_db)
...
@router.post(
        ...
        description="Add a new history record for a booking event"
    )
def create_history(
...
@router.put(
        ...
        discription="Update an existing booking history record by its ID."
    )
def update_history(
        history_id: int,
    ...
    if not updated:
        raise HTTPException(status_code=status.HTTP_404_NOT_CONTENT, detail="History not found")
    return updated
...
@router.delete(
        ...
        description="Remove a booking history record by its ID."
    )
def delete_history(
        history_id: int,
        db: Session = Depends(get_db)
    ):
    deleted = HistoryService(db).delete(history_id)
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_CONTENT, detail="History not found")
